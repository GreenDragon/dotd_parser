...
def ugup_request(table, game, platform, serverid=None):
    if serverid is None:
        serverid = 1
    ...
    for (id, platform, raid_id, raid_hash) in cursor:
        ...
        sql = "UPDATE %s SET update_time='%s', currenthealth='%s', enragehealth='%s', maxhealth='%s', debuff1id='%s', \
               ...
               nummembers='%s', maxattackers='%s', summonerid='%s', raid_boss_human='%s' WHERE id='%s';" \
              % ( table, update_time,
                  ...
                  raid_info['iscomplete'], raid_info['nummembers'], raid_info['maxattackers'],
                  raid_info['summonerid'], raid_info['raid_boss_human'],
                  id
                 )
...
if len(sys.argv) != 5:
    print "Usage: self {table_name} {game} {platform} {serverid}"
    print "Example: " + sys(argv[0]) + "dawn_shared_raids dawn facebook 1"
    exit()
else:
    if config['verbose_mode']:
        print "Working on: " + str(sys.argv)
    ugup_request( str(sys.argv[1]), str(sys.argv[2]), str(sys.argv[3]), int(sys.argv[4]) )
